...
4번 학생은 3번 학생이나 5번 학생에게만 체육복을 빌려줄 수 있습니다. 체육복이 없으면 수업을 들을 수 없기 때문에 체육복을 적절히 빌려 
...
def solution(n, lost, reserve):
    ...
    b=list(set(reserve)-set(lost)) #이 과정은 체육복을 여벌로 가지고 있는 학생이 체육복을 도난 당했을때의 예외처리 과정이다.
    
    answer = n - len(a) #체육복 여벌이 있는 친구가 다른 학생에체 체육복을 빌려주기전 체육 수업을 들을 수 있는 학생 수
    for i in a:
        if b.count(i-1):    #b배열 안에 i-1이 있다면 answer +1이후 b배열에서 해당 인덱스 삭제
