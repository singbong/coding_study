"""
문제
계단 오르기 게임은 계단 아래 시작점부터 계단 꼭대기에 위치한 도착점까지 가는 게임이다. <그림 1>과 같이 각각의 
...
if len(numbers)<= 2:
    ...
    d[2] = max(numbers[0]+numbers[2], numbers[1]+numbers[2])

    for i in range(3, n):        
        d[i]= max(d[i-2]+numbers[i], d[i-3]+numbers[i-1]+numbers[i])
