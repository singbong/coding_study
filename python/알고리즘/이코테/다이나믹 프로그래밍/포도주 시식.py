...
1부터 n까지의 번호가 붙어 있는 n개의 포도주 잔이 순서대로 테이블 위에 놓여 있고, 각 포도주 잔에 들어있는
...
if len(arr)<= 2:
    ...
    d[0]= arr[0]
    d[1]= max(arr[1], arr[0]+arr[1]) 
    d[2]= max(d[0]+arr[2], arr[2]+arr[1],d[1])
    for i in range(3, n):
        d[i]= max(d[i-2]+arr[i], d[i-3]+arr[i]+arr[i-1],d[i-1])
    print(d)
    print(d[n])